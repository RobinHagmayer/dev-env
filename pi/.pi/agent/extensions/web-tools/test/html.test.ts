...
test("html markdown conversion preserves whitespace spans in syntax-highlighted code", () => {
	const input = `<html><body><pre><code><span>pnpm</span><span> </span><span>add</span><span> </span><span>effect@beta</span></code></pre></body></html>`;
	assert.equal(htmlToMarkdown(input, "https://example.com/"), "```\npnpm add effect@beta\n```");
});
